...
type PanelTab = WorkbenchTab | 'file-preview';
type WorkbenchItem = { id: string; type: PanelTab; title: string; target?: string; filePath?: string; terminalId?: string };
...
const LABELS: Record<PanelTab, string> = { 'file-preview': '文件预览', files: '查看', browser: '浏览器', terminal: '终端', 'git-diff': 'Git Diff' };
...
export function TabletWorkbenchPanel(props: TabletWorkbenchPanelProps) {
  ...
  const select = (item: WorkbenchItem) => { setActiveId(item.id); props.onTabChange?.(item.type === 'file-preview' ? 'files' : item.type); };
  const open = (type: PanelTab, reuse = false) => {
    // Git diff is scoped to the conversation; terminals each have a real PTY ID.
    const existing = items.find(item => item.type === type);
    if (existing && (reuse || type === 'git-diff' || type === 'files')) { select(existing); return; }
    const item = { id: `${type}-${++counter.current}`, type, filePath: type === 'browser' ? props.browserFilePath : undefined, terminalId: type === 'terminal' ? (existing ? `${terminalIdForConversation(props.conversationId)}:${Date.now()}-${counter.current}` : terminalIdForConversation(props.conversationId)) : undefined, title: `${LABELS[type]} ${counter.current}`, target: type === 'files' || type === 'file-preview' ? props.initialFilePath : props.browserUrl };
    setItems(current => [...current, item]); select(item);
  };
  useEffect(() => {
    if (props.activeTab && (active?.type === 'file-preview' ? 'files' : active?.type) !== props.activeTab) open(props.activeTab, true);
  }, [props.activeTab]);
  useEffect(() => {
    setItems(current => {
  ...
  const preview = (path: string) => {
    ...
    const existing = items.find(item => item.type === 'file-preview' && item.target === path);
    if (existing) { select(existing); return; }
    const item: WorkbenchItem = { id: `file-preview-${++counter.current}`, type: 'file-preview', title: `预览 · ${path.split('/').pop()}`, target: path };
    setItems(current => [...current, item]); select(item);
  };
  ...
  return <View className="flex-1 bg-background">
    <View className="flex-row items-center border-b border-separator">
      <ScrollView horizontal className="flex-1" showsHorizontalScrollIndicator={false}>
        <Tabs value={activeId} onValueChange={id => { const item = items.find(item => item.id === id); if (item) select(item); }}>
          <Tabs.List className="gap-1 bg-transparent p-1">
            {items.map(item => <View key={item.id} className={`flex-row items-center rounded-xl ${item.id === activeId ? 'bg-accent/15' : 'bg-transparent'}`}>
              <Tabs.Trigger value={item.id} className="min-h-11"><Tabs.Label className={item.id === activeId ? 'text-accent font-semibold' : 'text-muted'}>{item.title}</Tabs.Label></Tabs.Trigger>
              <Button isIconOnly variant="ghost" className="h-11 w-11" accessibilityLabel={`关闭 ${item.title}`} isDisabled={items.length === 1} onPress={() => close(item)}><StyledIonicons name="close" size={16} className={item.id === activeId ? 'text-accent' : 'text-muted'} /></Button>
            </View>)}
          </Tabs.List>
        </Tabs>
      </ScrollView>
    ...
    {items.map(item => <View key={item.id} style={{ flex: 1, display: item.id === activeId ? 'flex' : 'none' }}>
      {item.type === 'files' && props.filesClient ? <FilesScreen client={props.filesClient} rootPath={props.workspace?.path || ''} initialFilePath={props.initialFilePath} onFileSelected={props.onEditFile} onPreview={preview} /> : null}
      {item.type === 'file-preview' && props.filesClient ? <FilePreviewScreen client={props.filesClient} path={item.target} visible={props.visible !== false && item.id === activeId} onEdit={props.onEditFile} /> : null}
      ...
      {((item.type === 'files' || item.type === 'file-preview') && !props.filesClient) || (item.type === 'browser' && !props.browserClient) ? <EmptyStateView title="后端未连接" description="连接后重新打开工作台。" icon="cloud-offline-outline" /> : null}
    </View>)}
    <ActionSheet isOpen={adding} onOpenChange={setAdding} title="新建工作台标签" actions={([...WORKBENCH_TABS, 'file-preview'] as PanelTab[]).map(type => ({ id: type, label: LABELS[type], onPress: () => { setAdding(false); open(type); } }))} />
  </View>;
}
